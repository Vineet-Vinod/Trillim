...
"""
Opt-in live integration tests for the Trillim OpenAI-compatible API server.

These tests require an already-running server plus real model assets. Run them with:
...
Voice tests run automatically when the server has `--voice` enabled; otherwise
they are skipped. LoRA tests require `ADAPTER_DIR` pointing to a directory with
`qmodel.lora` (separate from `MODEL_DIR`).
"""
...
_BASE_URL_ENV = "TRILLIM_LIVE_SERVER_BASE_URL"
...
def stream_chunks(base_url: str, path: str, body: dict, timeout: int = 300):
    """Make a streaming request and return parsed SSE data and DONE status."""
    url = f"{base_url}{path}"
...
def api_binary(base_url: str, method: str, path: str, body=None, timeout: int = 300):
...
def _hot_swap_allowed(model_dir: str) -> bool:
    """Return True if model_dir resolves under ~/.trillim/models/."""
...
def test_chat_streaming(base_url: str, **_):
    ...
    content_pieces = [c["choices"][0]["delta"].get("content", "") for c in chunks[1:-1]]
...
def test_load_model_with_search_harness_and_provider(base_url: str, model_dir: str | None = None, **_):
    ...
    if model_dir is None:
        return "skip", "no MODEL_DIR provided"
    ...
    chat_payload = {"messages": [{"role": "user", "content": "Say hi."}], "max_tokens": 32}
...
def test_search_harness_stream_hides_internal_markers(base_url: str, model_dir: str | None = None, **_):
    ...
    chunks, got_done = stream_chunks(
        base_url,
        "/v1/chat/completions",
        {"messages": [{"role": "user", "content": "Say hi."}], "max_tokens": 32, "stream": True},
    )
    if not got_done:
        return "fail", "stream did not end with data: [DONE]"
...
def test_completions_non_streaming(base_url: str, **_):
    ...
    payload = {"prompt": "Once upon a time", "max_tokens": 32}
    status, body = api(base_url, "POST", "/v1/completions", payload)
    if status != 200:
        return "fail", f"expected 200, got {status}"
...
def test_completions_streaming(base_url: str, **_):
    ...
    payload = {"prompt": "The sky is", "max_tokens": 32, "stream": True}
...
def test_max_tokens(base_url: str, **_):
    ...
    payload = {"messages": [{"role": "user", "content": "Tell me a long story."}], "max_tokens": 5}
    status, body = api(base_url, "POST", "/v1/chat/completions", payload)
    if status != 200:
        return "fail", f"expected 200, got {status}"
...
def test_cached_tokens_field(base_url: str, **_):
    ...
    payload = {"messages": [{"role": "user", "content": "What color is the sky?"}], "max_tokens": 16}
    status, body = api(base_url, "POST", "/v1/chat/completions", payload)
    if status != 200:
        return "fail", f"expected 200, got {status}"
    usage = body.get("usage", {})
...
def test_cache_miss_different_conversation(base_url: str, **_):
    ...
    api(
        ...
        "POST",
        "/v1/chat/completions",
        {"messages": [{"role": "user", "content": "Tell me about dogs."}], "max_tokens": 8},
    )
    status, body = api(
        base_url,
        "POST",
        "/v1/chat/completions",
        {"messages": [{"role": "user", "content": "Tell me about quantum physics."}], "max_tokens": 8},
    )
    if status != 200:
        return "fail", f"expected 200, got {status}"
...
def test_cache_hit_four_turns(base_url: str, **_):
    ...
    prev_cached = cached1
    for turn, user_msg in enumerate(["Now count from 4 to 6.", "Now 7 to 9.", "And finally 10."], start=2):
        messages += [
        ...
        if cached <= prev_cached:
            return "fail", f"turn {turn}: cached_tokens should grow as conversation extends (prev={prev_cached}, now={cached})"
        prev_cached = cached


def test_cache_reset_then_resume(base_url: str, **_):
    """Interruption resets cache; resumed conversation should then hit cache."""
    msgs_a = [{"role": "user", "content": "What is the largest ocean?"}]
    status, body = api(base_url, "POST", "/v1/chat/completions", {"messages": msgs_a, "max_tokens": 16, "temperature": 0})
    if status != 200:
        return "fail", f"turn 1 failed: {status}"
    ...
    status, body = api(base_url, "POST", "/v1/chat/completions", {"messages": msgs_b, "max_tokens": 8, "temperature": 0})
    if status != 200:
    ...
    if cached4 <= cached3:
        return "fail", f"turn 4: cached_tokens should exceed turn 3 (turn3={cached3}, turn4={cached4})"


def test_cache_interleaved_clients(base_url: str, **_):
    """Interleaved client requests should produce misses then hits as cache moves."""
    msgs_a = [{"role": "user", "content": "What is the tallest mountain?"}]
    s, body = api(base_url, "POST", "/v1/chat/completions", {"messages": msgs_a, "max_tokens": 16, "temperature": 0})
    if s != 200:
    ...
    s, body = api(base_url, "POST", "/v1/chat/completions", {"messages": msgs_b, "max_tokens": 16, "temperature": 0})
    if s != 200:
    ...
    s, body = api(base_url, "POST", "/v1/chat/completions", {"messages": msgs_a2, "max_tokens": 16, "temperature": 0})
    if s != 200:
...
def test_completions_cached_tokens(base_url: str, **_):
    ...
    payload = {"prompt": "Once upon a time in a land far away", "max_tokens": 8}
    status, body = api(base_url, "POST", "/v1/completions", payload)
    if status != 200:
        return "fail", f"expected 200, got {status}"
    usage = body.get("usage", {})
...
def test_cache_cleared_after_hot_swap(base_url: str, model_dir: str | None = None, **_):
    ...
    api(base_url, "POST", "/v1/chat/completions", {"messages": msg, "max_tokens": 8})
    swap_status, swap_body = api(base_url, "POST", "/v1/models/load", {"model_dir": model_dir}, timeout=600)
    if swap_status != 200:
        return "fail", f"hot-swap failed: {swap_status}: {swap_body}"

    status, body = api(base_url, "POST", "/v1/chat/completions", {"messages": msg, "max_tokens": 8})
    if status != 200:
...
def test_load_model(base_url: str, model_dir: str | None = None, **_):
    ...
    status, body = api(base_url, "POST", "/v1/models/load", {"model_dir": model_dir}, timeout=600)
    if status != 200:
        return "fail", f"expected 200, got {status}: {body}"
    if body["status"] != "success":
        return "fail", f"expected success, got {body}"
    ...
    status2, body2 = api(base_url, "POST", "/v1/chat/completions", {"messages": [{"role": "user", "content": "Say hi."}], "max_tokens": 16})
    if status2 != 200:
...
def test_load_invalid_model(base_url: str, **_):
    """POST /v1/models/load with a nonexistent path returns an error."""
    status, body = api(base_url, "POST", "/v1/models/load", {"model_dir": "/tmp/nonexistent_model_dir_12345"})
    if status not in (403, 404, 500):
        return "fail", f"expected 403/404/500 for invalid model, got {status}: {body}"


def test_load_model_with_adapter(base_url: str, model_dir: str | None = None, adapter_dir: str | None = None, **_):
    """POST /v1/models/load with adapter_dir loads LoRA from a separate directory."""
    if model_dir is None:
        return "skip", "no MODEL_DIR provided"
    if adapter_dir is None:
        return "skip", "no ADAPTER_DIR provided"
    if not _hot_swap_allowed(model_dir):
        return "skip", "model not in ~/.trillim/models/ (use 'trillim pull' for hot-swap tests)"
    ...
    status2, body2 = api(base_url, "POST", "/v1/chat/completions", {"messages": [{"role": "user", "content": "Say hi."}], "max_tokens": 32})
    if status2 != 200:
...
def test_swap_adapter_to_no_adapter(base_url: str, model_dir: str | None = None, adapter_dir: str | None = None, **_):
    ...
    status3, body3 = api(base_url, "POST", "/v1/chat/completions", {"messages": [{"role": "user", "content": "Say hi."}], "max_tokens": 16})
    if status3 != 200:
...
def test_concurrent_swap_conflict(base_url: str, model_dir: str | None = None, **_):
    """Two rapid concurrent swaps: both should succeed or one may conflict."""
    if model_dir is None:
        return "skip", "no MODEL_DIR provided"
    if not _hot_swap_allowed(model_dir):
        return "skip", "model not in ~/.trillim/models/ (use 'trillim pull' for hot-swap tests)"
    ...
    def _swap(idx):
        try:
            s, b = api(base_url, "POST", "/v1/models/load", {"model_dir": model_dir}, timeout=600)
    ...
    for i, result in enumerate(results):
        if "error" in result:
            return "fail", f"swap {i} raised an exception: {result['error']}"
        if result.get("status") not in (200, 409):
            return "fail", f"swap {i} unexpected status: {result}"
    if not any(result.get("status") == 200 for result in results):
        return "fail", f"no swap succeeded: {results}"


def test_inference_during_swap(base_url: str, model_dir: str | None = None, **_):
    """Swap completes and inference works immediately after."""
    if model_dir is None:
        return "skip", "no MODEL_DIR provided"
    if not _hot_swap_allowed(model_dir):
        return "skip", "model not in ~/.trillim/models/ (use 'trillim pull' for hot-swap tests)"
    ...
    status2, _ = api(base_url, "POST", "/v1/chat/completions", {"messages": [{"role": "user", "content": "Hello"}], "max_tokens": 8})
    if status2 != 200:
        return "fail", f"post-swap inference failed: {status2}"
...
def test_voices_list(base_url: str, **_):
    ...
    predefined = [voice for voice in voices if voice["type"] == "predefined"]
    if len(predefined) != 8:
        return "fail", f"expected 8 predefined, got {len(predefined)}"

    names = {voice["voice_id"] for voice in predefined}
    for expected in ("alba", "marius", "javert", "jean", "fantine", "cosette", "eponine", "azelma"):
...
def test_tts_wav(base_url: str, **_):
    ...
    status, data, _ = api_binary(base_url, "POST", "/v1/audio/speech", {"input": "Hello world, this is a test."})
    if status != 200:
        return "fail", f"expected 200, got {status}"
...
def test_tts_pcm(base_url: str, **_):
    ...
    status, data, _ = api_binary(base_url, "POST", "/v1/audio/speech", {"input": "Testing raw PCM output.", "response_format": "pcm"})
    if status != 200:
        return "fail", f"expected 200, got {status}"
    if data[:4] == b"RIFF":
...
def test_tts_voice_selection(base_url: str, **_):
    ...
    status, data, _ = api_binary(base_url, "POST", "/v1/audio/speech", {"input": "Voice selection test.", "voice": "jean"})
    if status != 200:
        return "fail", f"expected 200, got {status}"
    if not _is_valid_wav(data):
        return "fail", "response is not a valid WAV file"
...
def test_tts_wav_speed(base_url: str, **_):
    ...
    text = "Speed test. " * 12
    normal_status, normal_data, _ = api_binary(base_url, "POST", "/v1/audio/speech", {"input": text, "speed": 1.0})
    fast_status, fast_data, _ = api_binary(base_url, "POST", "/v1/audio/speech", {"input": text, "speed": 2.0})
...
def test_tts_pcm_speed(base_url: str, **_):
    ...
    normal_status, normal_data, _ = api_binary(base_url, "POST", "/v1/audio/speech", {"input": text, "response_format": "pcm", "speed": 1.0})
    slow_status, slow_data, _ = api_binary(base_url, "POST", "/v1/audio/speech", {"input": text, "response_format": "pcm", "speed": 0.5})
...
def test_tts_invalid_speed(base_url: str, **_):
    ...
    status, _, _ = api_binary(base_url, "POST", "/v1/audio/speech", {"input": "Hello world.", "speed": 4.5})
    if status != 400:
...
def test_tts_empty_input(base_url: str, **_):
    ...
    status, _, _ = api_binary(base_url, "POST", "/v1/audio/speech", {"input": "   "})
    if status != 400:
...
def test_stt_roundtrip(base_url: str, **_):
    ...
    status, wav_data, _ = api_binary(base_url, "POST", "/v1/audio/speech", {"input": "The quick brown fox jumps over the lazy dog."})
    if status != 200:
...
def test_stt_text_format(base_url: str, **_):
    ...
    status, wav_data, _ = api_binary(base_url, "POST", "/v1/audio/speech", {"input": "Hello."})
    if status != 200:
        return "fail", f"TTS failed: {status}"
...
def test_custom_voice_lifecycle(base_url: str, **_):
    ...
    status, wav_data, _ = api_binary(base_url, "POST", "/v1/audio/speech", {"input": "This is my custom voice sample for cloning purposes."})
    if status != 200:
        return "fail", f"TTS failed: {status}"
    ...
    voices = body["voices"]
    custom = [voice for voice in voices if voice["voice_id"] == voice_id]
    if len(custom) != 1:
        return "fail", "custom voice not found in list"
    if custom[0]["type"] != "custom":
        return "fail", f"expected type=custom, got {custom[0]['type']}"

    status, audio, _ = api_binary(base_url, "POST", "/v1/audio/speech", {"input": "Testing custom voice.", "voice": voice_id})
    if status != 200:
    ...
    req = urllib.request.Request(f"{base_url}/v1/voices/{voice_id}", method="DELETE")
    try:
    ...
    if any(voice["voice_id"] == voice_id for voice in body["voices"]):
...
def test_delete_nonexistent_voice(base_url: str, **_):
    ...
    req = urllib.request.Request(f"{base_url}/v1/voices/nonexistent_voice_xyz", method="DELETE")
    try:
...
def test_delete_predefined_voice(base_url: str, **_):
    ...
    req = urllib.request.Request(f"{base_url}/v1/voices/alba", method="DELETE")
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            status = resp.status
    except urllib.error.HTTPError as exc:
...
def test_chat_still_works_with_voice(base_url: str, **_):
    ...
    payload = {"messages": [{"role": "user", "content": "Say hi."}], "max_tokens": 16}
    status, body = api(base_url, "POST", "/v1/chat/completions", payload)
    if status != 200:
        return "fail", f"expected 200, got {status}"
    content = body["choices"][0]["message"]["content"]
...
class ServerApiLiveTests(_LegacyServerTestCase):
    """Live integration tests for chat, completion, cache, and model loading."""


class ServerVoiceLiveTests(_LegacyServerTestCase):
    """Live integration tests for TTS, STT, and voice-management routes."""
...
_install_test_methods(ServerApiLiveTests, _API_TEST_FUNCTIONS)
_install_test_methods(ServerVoiceLiveTests, _VOICE_TEST_FUNCTIONS)


def load_tests(loader, tests, pattern):
    suite = unittest.TestSuite()
    for case_cls, test_functions in (
        (ServerApiLiveTests, _API_TEST_FUNCTIONS),
        (ServerVoiceLiveTests, _VOICE_TEST_FUNCTIONS),
    ):
        for test_fn in test_functions:
...
def _load_test_config() -> None:
    _TEST_CONFIG.base_url = os.getenv(_BASE_URL_ENV, "http://127.0.0.1:8000")
...
def _ensure_server_reachable() -> None:
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not connect to {base}. Start the server first, then re-run the live suite."
        ) from None
